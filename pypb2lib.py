...
def get_player_info(nameorid):
    nicks = []
    try:
        dp_id = int(nameorid)
        res = urlopen('http://dplogin.com/index.php?action=viewmember&playerid={id}'.format(id=nameorid)).read()
        names = re.findall(
            '\\<tr\\>\\<td\\>\\<b\\ class\\=\\"faqtitle\\"\\>Names?\\ registered\\:\\<\\/b\\>\\<\\/td\\>\\<td\\>(.*?)\\<\\/td\\>\\<\\/tr\\>',
            res)[0]
    except:
        response = urlopen(
            'http://dplogin.com/index.php?action=displaymembers&search={name}'.format(name=nameorid)).read()
        matches = re.findall(
            '\\<a\\ href\\=\\"\\/index\\.php\\?action\\=viewmember\\&playerid\\=(\d+)\\"\\>.*?\\<\\/a\\>', response)
        full_list = []
        response_list = []
        for i in matches:
...
class Server():
    ...
    def rcon(self, command=None):
        ...
        sock.settimeout(3)
        if command != None:
            sock.send((b"\xFF\xFF\xFF\xFF" + ('rcon %s %s' % (self.__rcon_password, command)).encode()))
        else:
            sock.send(b"\xFF\xFF\xFF\xFFstatus\0")
        try:
    ...
    def get_status(self):
        ...
        for i in players_:
            temp_dict = {}
            cleaned_name = clean_special_chars(i)
    ...
    def _on_event(self, text):
        ...
        # Elimination
        elim = re.findall('^\\[\d\d\\:\d\d\\:\d\d\\]\\ \\*(.*?)\\ \\((.*?)\\)\\ eliminated\\ \\*(.*?)\\ \\((.*?)\\).',
                          text) # [18:54:24] *|ACEBot_1| (Spyder SE) eliminated *|herself| (Spyder SE).
        if len(elim) != 0:
            arg = [time.time()]
            arg.extend(list(elim[0]))
            arg = list_to_dict(arg)
            arg['time'] = arg[0]
            arg['player1'] = arg[1]
            arg['gun1'] = arg[2]
            arg['player2'] = arg[3]
        ...
        # Respawn
        respawn = re.findall("^\\[\d\d\\:\d\d\\:\d\d\\]\\ \\*(.*?)\\'s\\ (.*?)\\ revived\\!",
                             text)  # [19:03:57] *Red's ACEBot_6 revived!
        if len(respawn) != 0:
            arg = [time.time()]
        ...
        # Chat
        chat = re.findall("^\\[\d\d\\:\d\d\\:\d\d\\]\\ (.*?)\\: (.+)", text) # [19:54:18] hTml: test
        if len(chat) != 0:
            arg = [time.time()]
            ...
            arg['player'] = arg[1]
            arg['text'] = arg[2]
            playerlist = self.get_simple_playerlist()
        ...
        # Map change
        mapchange = re.findall("^\\[\d\d\\:\d\d\\:\d\d\\]\\ \\=\\=\\ Map Loaded\\:\\ (.*?)\\ \\=\\=",
                               text)  # [19:49:17] == Map Loaded: propaint1 ==
        if len(mapchange) != 0:
            arg = [time.time()]
    ...
    def get_players_ingame_id(self, name):
